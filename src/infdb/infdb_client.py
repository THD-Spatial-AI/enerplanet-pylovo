    ...
    def fetch_buildings_from_infdb(self, plz: int) -> list[tuple]:
        ...
        Returns:
            list[tuple]: A list of tuples, where each tuple contains:
                ...
                - building_type (str): Type of building (e.g., 'house', 'apartments', 'office')
        ...
        query = """
            SELECT id,
                   floor_area,
                   COALESCE(NULLIF(TRIM(building_type), ''), NULLIF(TRIM(building_use), ''), 'building') as type,
                   COALESCE(NULLIF(TRIM(building_type), ''), NULLIF(TRIM(building_use), ''), 'yes') as f_class,
