...
class ParameterCalculator:
    ...
    def calc_parameters_per_plz(self, plz):
        grid_generated = self.dbc.is_grid_generated(plz, self.country_code)
    ...
    def calculate_line_with_sim_factor(self, pandapower_net, networkx_graph) -> pd.DataFrame:
        ...
        # Capitalize descriptions to match zone values
            ...
            try:
                ...
                def _normalize_bus_zone(zone_val):
                    if not isinstance(zone_val, str):
                        return "commercial"
                    raw = zone_val.strip().lower()
                    mapped = _zone_parent_pc.get(raw, raw)
                    return mapped if mapped in _sim_factor_map else "commercial"

                _bus_zones_pc = net.bus["zone"].map(_normalize_bus_zone)

                sim_peak_load = 0.0
                for building_type, sim_factor in _sim_factor_map.items():
                    bus_index = net.bus[_bus_zones_pc == building_type].index.tolist()
                    if not bus_index:
                        continue
