...
class ParameterCalculator:
    ...
    def __init__(self, keyword_lvbus: str = "LVbus", keyword_consumer_bus: str = "Consumer Nodebus",
            keyword_connection_bus: str = "Connection Nodebus", country_code: str = "DE"):
    ...
    def calc_parameters_per_plz(self, plz = None):
        ...
        self.plz = plz
        grid_generated = self.dbc.is_grid_generated(self.plz, self.country_code)
    ...
    def calculate_line_with_sim_factor(self, pandapower_net, networkx_graph) -> pd.DataFrame:
        ...
        # Capitalize descriptions to match zone values (e.g., 'residential' -> 'Residential')
        df_sim_factor_definitions['description'] = df_sim_factor_definitions['description'].str.capitalize()

        # Build f_class -> parent_category lookup from CONSUMER_CATEGORIES
        _cc = CONSUMER_CATEGORIES if 'parent_category' in CONSUMER_CATEGORIES.columns else pd.DataFrame()
        _fclass_to_parent = (
            _cc.assign(definition=_cc['definition'].astype(str).str.strip().str.lower())
            .set_index('definition')['parent_category']
            .astype(str).str.strip().str.lower()
            .to_dict()
            if not _cc.empty else {}
        )
        _sim_factor_categories = {str(k).strip().lower() for k in SIM_FACTOR.keys()}
        ...
        # Map f_class zones to parent_category and keep only configured SIM_FACTOR categories.
        # Unknown categories fall back to commercial.
        def _normalize_zone(zone_val):
            if not isinstance(zone_val, str):
                return "commercial"
            raw = zone_val.strip().lower()
            mapped = _fclass_to_parent.get(raw, raw)
            if mapped not in _sim_factor_categories:
                return "commercial"
            return mapped

        level1['zone'] = level1['zone'].map(_normalize_zone).str.capitalize()
        ...
        for index, row in load_count_cat.iterrows():
            ...
            if row['description'] in ('Commercial', 'Industrial', 'Agricultural'):
            ...
            elif row['description'] in ('Public', 'Infrastructure'):
        ...
        for index, row in df_connection_bus.iterrows():
            ...
            load_commercial = utils.oneSimultaneousLoad(
                ...
                sim_factor=SIM_FACTOR.get('commercial', SIM_FACTOR.get('Commercial', 0.50)))

            load_public = utils.oneSimultaneousLoad(
                installed_power=net_line_with_sim_factor.at[upstream_index[0], 'load_public_mw'],
                load_count=net_line_with_sim_factor.at[upstream_index[0], 'no_public'], sim_factor=SIM_FACTOR.get('public', SIM_FACTOR.get('Public', 0.60)))
            ...
            load_residential = utils.oneSimultaneousLoad(
                ...
                sim_factor=SIM_FACTOR.get('residential', SIM_FACTOR.get('Residential', 0.07)))
    ...
    def analyse_basic_parameters_per_plz(self, plz: int):
        ...
        self.dbc.insert_plz_parameters(plz, trafo_string, load_count_string, bus_count_string, self.country_code)
    ...
    def analyse_trafo_parameters_per_plz(self, plz: int):
        ...
        for kcid, bcid in cluster_list:
            try:
                ...
                # category partitions using parent_category from consumer_categories
                _zone_parent = (
                    CONSUMER_CATEGORIES.assign(
                        definition=CONSUMER_CATEGORIES['definition'].astype(str).str.strip().str.lower(),
                        parent_category=CONSUMER_CATEGORIES['parent_category'].astype(str).str.strip().str.lower(),
                    )
                    .set_index('definition')['parent_category'].to_dict()
                    if 'parent_category' in CONSUMER_CATEGORIES.columns else {}
                )
                _sim_factor_map = {str(k).strip().lower(): float(v) for k, v in SIM_FACTOR.items()}

                def _normalize_bus_zone(zone_val):
                    if not isinstance(zone_val, str):
                        return "commercial"
                    raw = zone_val.strip().lower()
                    mapped = _zone_parent.get(raw, raw)
                    return mapped if mapped in _sim_factor_map else "commercial"

                _bus_zones = net.bus["zone"].map(_normalize_bus_zone)

                sim_peak_load = 0.0
                for building_type, sim_factor in _sim_factor_map.items():
                    bus_index = net.bus[_bus_zones == building_type].index.tolist()
                    if not bus_index:
                        continue
                    house_num = net.load[net.load["bus"].isin(bus_index)].shape[0]
                    if house_num == 0:
                        continue
                    sum_load = net.load[net.load["bus"].isin(bus_index)]["max_p_mw"].sum() * 1e3
