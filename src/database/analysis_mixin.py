...
class AnalysisMixin(BaseMixin, ABC):
    ...
    def insert_plz_parameters(self, plz, trafo_string: str, load_count_string: str, bus_count_string: str, country_code: str = "DE"):
        update_query = """INSERT INTO plz_parameters (version_id, plz, country_code, trafo_num, load_count_per_trafo, bus_count_per_trafo)
                          VALUES (%s, %s, %s, %s, %s,
                                  %s);"""  # TODO: check - should values be updated for same plz and version if analysis is started? And Add a column
        self.cur.execute(update_query, vars=(VERSION_ID, str(plz), country_code, trafo_string, load_count_string, bus_count_string), )
