...
class PreprocessingMixin(BaseMixin, ABC):
    ...
    def ensure_shared_table_columns(self):
        """Add f_classes and enrichment columns to shared tables (res, oth) if missing.

        Must be called ONCE from the main process before spawning workers.
        ALTER TABLE requires ACCESS EXCLUSIVE lock, so running it from
        multiple workers simultaneously causes deadlocks.
        """
        self.cur.execute("ALTER TABLE res ADD COLUMN IF NOT EXISTS f_classes text;")
        self.cur.execute("ALTER TABLE oth ADD COLUMN IF NOT EXISTS f_classes text;")
        # Enrichment columns from 3D BAG / EP-Online
        for tbl in ("res", "oth"):
            for col, dtype in [
                ("height_max", "double precision"), ("height_ground", "double precision"),
                ("height_median", "double precision"), ("floors_3dbag", "integer"),
                ("bag_id", "varchar"), ("energy_label", "varchar(5)"),
                ...
                self.cur.execute(f"ALTER TABLE {tbl} ADD COLUMN IF NOT EXISTS {col} {dtype};")
        self.conn.commit()
    ...
    def insert_version_if_not_exists(self):
        ...
        try:
            ...
            other_parameters_dict = {
                ...
                "WAYS_BOUNDARY_BUFFER_M": WAYS_BOUNDARY_BUFFER_M,
                "SMALL_COMPONENT_MERGE_ENABLED": SMALL_COMPONENT_MERGE_ENABLED,
                "SMALL_COMPONENT_MERGE_MAX_BUILDINGS": SMALL_COMPONENT_MERGE_MAX_BUILDINGS,
                "SMALL_COMPONENT_MERGE_MAX_DISTANCE_M": SMALL_COMPONENT_MERGE_MAX_DISTANCE_M,
                "KCID_QA_GATES_ENABLED": KCID_QA_GATES_ENABLED,
                "KCID_QA_MAX_SINGLETON_RATIO": KCID_QA_MAX_SINGLETON_RATIO,
                "KCID_QA_MAX_UNASSIGNED_CONSUMERS": KCID_QA_MAX_UNASSIGNED_CONSUMERS,
                "VN": VN,
            ...
            # Drop obsolete unique constraint on transformer_positions if it exists
            # This allows multiple clusters to share the same OSM transformer
            # Uses IF EXISTS to avoid lock contention when run from multiple workers
            try:
                self.cur.execute("""
                    DO $$ BEGIN
                        IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_tp_osm_v') THEN
                            ALTER TABLE transformer_positions DROP CONSTRAINT uq_tp_osm_v;
                        END IF;
                    END $$""")
                self.conn.commit()
            except Exception as e:
                self.logger.debug(f"Note: Could not drop constraint uq_tp_osm_v (might not exist yet): {e}")
                self.conn.rollback()
    ...
    def insert_equipment_data_from_config(self, equipment_data: pd.DataFrame):
        ...
        """
        self.cur.execute("ALTER TABLE equipment_data ADD COLUMN IF NOT EXISTS equipment_only_cost_eur integer")
        self.cur.execute("ALTER TABLE equipment_data ADD COLUMN IF NOT EXISTS installed_cost_eur integer")
        self.conn.commit()

        df = equipment_data.copy()
        expected_cols = ["version_id", "name", "s_max_kva", "max_i_a", "r_mohm_per_km", "x_mohm_per_km",
                         "z_mohm_per_km", "cost_eur", "equipment_only_cost_eur", "installed_cost_eur", "typ"]
        ...
        if "equipment_only_cost_eur" in df.columns:
            df["equipment_only_cost_eur"] = df["equipment_only_cost_eur"].where(
                df["equipment_only_cost_eur"].notna(),
                df["cost_eur"],
            )
        ...
        try:
            values = [
                (r['version_id'], r['name'], r['s_max_kva'], r['max_i_a'], r['r_mohm_per_km'],
                 r['x_mohm_per_km'], r['z_mohm_per_km'], r['cost_eur'],
                 r['equipment_only_cost_eur'], r['installed_cost_eur'], r['typ'])
                for r in rows
            ]
            execute_values(
                self.cur,
                """INSERT INTO equipment_data
                   (version_id, name, s_max_kva, max_i_a, r_mohm_per_km, x_mohm_per_km, z_mohm_per_km,
                    cost_eur, equipment_only_cost_eur, installed_cost_eur, typ)
                   VALUES %s
                   ON CONFLICT (version_id, name) DO UPDATE SET s_max_kva        = EXCLUDED.s_max_kva,
                                                                ...
                                                                equipment_only_cost_eur = EXCLUDED.equipment_only_cost_eur,
                                                                installed_cost_eur = EXCLUDED.installed_cost_eur,
                                                                typ              = EXCLUDED.typ""",
                values,
                page_size=100,
            )
    ...
    def insert_consumer_categories_from_config(self, consumer_categories: pd.DataFrame):
        ...
        expected_cols = ["consumer_category_id", "definition", "load_method", "parent_category",
                         "peak_load", "yearly_consumption", "peak_load_per_m2",
                         "yearly_consumption_per_m2", "sim_factor"]
        for col in expected_cols:
            if col not in df.columns:
                df[col] = None
        # Fill defaults for new columns
        df['load_method'] = df['load_method'].fillna('area')
        df['parent_category'] = df['parent_category'].fillna('commercial')
        df = df[expected_cols]
        ...
        try:
            values = [
                (r['consumer_category_id'], r['definition'], r['load_method'], r['parent_category'],
                 r['peak_load'], r['yearly_consumption'],
                 r['peak_load_per_m2'], r['yearly_consumption_per_m2'], r['sim_factor'])
                for r in rows
            ]
            execute_values(
                self.cur,
                """INSERT INTO consumer_categories
                   (consumer_category_id, definition, load_method, parent_category, peak_load, yearly_consumption,
    ...
    def copy_postcode_result_table(self, plz: int, country_code: str = "DE") -> None:
        """
        Copies the given plz entry from postcode to the postcode_result table
        :param plz: postcode
        :param country_code: country code (e.g., 'DE', 'NL')
        :return:
        """
        query = """INSERT INTO postcode_result (version_id, postcode_result_plz, country_code, state_code, geom)
                   SELECT %(v)s as version_id, plz, country_code, state_code, geom
                   FROM postcode
                   WHERE plz = %(p)s AND country_code = %(c)s
                   LIMIT 1
                   ON CONFLICT (version_id, postcode_result_plz, country_code) 
                   DO UPDATE SET state_code = COALESCE(EXCLUDED.state_code, postcode_result.state_code);"""

        self.cur.execute(query, {"v": VERSION_ID, "p": plz, "c": country_code})

    def set_residential_buildings_table(self, plz: int, country_code: str = "DE"):
        """
        * Fills buildings_tem with residential buildings that lie inside the postal code geometry
        ...
        # `buildings_tem` is exposed as a temp VIEW per PLZ, so alter the
        # backing table directly.
        self.cur.execute(f"ALTER TABLE IF EXISTS buildings_tem_{plz} ADD COLUMN IF NOT EXISTS f_classes text;")
        for col, dtype in [
            ("cbs_population", "double precision"), ("cbs_households", "double precision"),
            ("cbs_avg_household_size", "double precision"),
        ]:
            self.cur.execute(f"ALTER TABLE IF EXISTS buildings_tem_{plz} ADD COLUMN IF NOT EXISTS {col} {dtype};")
        # NOTE: ALTER TABLE res is done once in ensure_shared_table_columns() before workers spawn.

        # Fill table - include occupants as households_per_building from datapipeline data
        query = """INSERT INTO buildings_tem (osm_id, area, type, f_class, f_classes, geom, center, floors, households_per_building, plz,
                   ...
                   SELECT osm_id, area, building_t, f_class, COALESCE(NULLIF(f_classes, ''), f_class),
                          res.geom, ST_Centroid(res.geom), floors::int, occupants::int, %(plz)s,
                          res.height_max, res.height_ground, res.height_median, res.floors_3dbag, res.bag_id, res.energy_label, res.energy_index,
                          res.cbs_population, res.cbs_households, res.cbs_avg_household_size, res.constructi
                   FROM res
                   JOIN postcode_result post ON post.geom && res.geom
                                            AND ST_Intersects(post.geom, res.geom)
                   WHERE post.version_id = %(v)s
                     AND post.postcode_result_plz = %(plz)s
                     AND post.country_code = %(c)s
                     AND res.country_code = %(c)s
                     AND ST_Area(res.geom) >= 25;"""
        self.cur.execute(query, {"v": VERSION_ID, "plz": plz, "c": country_code})
    ...
    def set_buildings_table(self, buildings_data: list[tuple], plz: int = None) -> None:
        ...
        execute_values(
            ...
            """INSERT INTO buildings_tem
               (osm_id, area, type, f_class, geom, center, floors, households_per_building, address_street_id, construction_year)
               VALUES %s""",
            buildings_data,
            template="(%s, %s, %s, %s, ST_Transform(%s::geometry, 3035), ST_Transform(%s::geometry, 3035), %s, %s, %s, %s)",
            page_size=500,
        )
    ...
    def set_buildings_table_with_geometry_filter(self, buildings_data: list[tuple], allocated_plz: int) -> None:
        ...
        self.cur.execute("""
            INSERT INTO buildings_tem
            (osm_id, area, type, f_class, geom, center, floors, households_per_building, address_street_id, construction_year)
            SELECT tb.osm_id, tb.area, tb.type, tb.f_class, tb.geom, tb.center, tb.floors, 
    ...
    def set_other_buildings_table(self, plz: int, country_code: str = "DE"):
        """
        ...
        * Sets all floors to 1 if missing
        ...
        for col, dtype in [
            ...
            self.cur.execute(f"ALTER TABLE IF EXISTS buildings_tem_{plz} ADD COLUMN IF NOT EXISTS {col} {dtype};")
        # NOTE: ALTER TABLE oth is done once in ensure_shared_table_columns() before workers spawn.

        # Fill table
        query = """INSERT INTO buildings_tem(osm_id, area, type, f_class, f_classes, geom, center, plz, floors,
                                            height_max, height_ground, height_median, floors_3dbag, bag_id,
                                            cbs_population, cbs_households, cbs_avg_household_size, construction_year)
                   SELECT osm_id, area, use, f_class, COALESCE(NULLIF(f_classes, ''), f_class),
                          o.geom, ST_Centroid(o.geom), %(plz)s, 1,
                          o.height_max, o.height_ground, o.height_median, o.floors_3dbag, o.bag_id,
                          o.cbs_population, o.cbs_households, o.cbs_avg_household_size, NULL::text
                   FROM oth AS o
                   JOIN postcode_result post ON post.geom && o.geom
                                            AND ST_Intersects(post.geom, o.geom)
                   WHERE post.version_id = %(v)s
                     AND post.postcode_result_plz = %(plz)s
                     AND post.country_code = %(c)s
                     AND o.country_code = %(c)s
                     AND ST_Area(o.geom) >= 25;
    ...
    def remove_duplicate_buildings(self):
        ...
        * Remove buildings that are duplicates (copy id) of others
        :return:
        """
        query = """DELETE FROM buildings_tem WHERE geom IS NULL;
                   DELETE FROM buildings_tem WHERE osm_id IS NULL;
    ...
    def calculate_house_distance_metric(
        self,
        plz: int,
        country_code: str = "DE",
        sample_size: int = 50,
        ...
        """Computes the average inter-building distance (meters) based on a deterministic sample
        ...
        distance_query = """WITH some_buildings AS (
                                ...
                                FROM buildings_tem
                                ORDER BY md5(COALESCE(osm_id, ST_AsText(center)))
                                LIMIT %(sample_size)s
                            )
                            ...
                            FROM some_buildings AS b
                                ...
                                LIMIT %(k_nearest)s) AS d
                                               ON TRUE;"""
        self.cur.execute(distance_query, {"sample_size": int(sample_size), "k_nearest": int(k_nearest)})
        ...
        self.cur.execute(update_query, {"avg": avg_dis, "v": VERSION_ID, "p": plz, "cc": country_code})
        return avg_dis

    def calculate_avg_households_per_building(self, plz: int, country_code: str = "DE") -> float:
        ...
        """
        avg_query = """
            SELECT AVG(b.households_per_building)::DOUBLE PRECISION
            FROM buildings_tem b
            JOIN consumer_categories cc ON cc.definition = b.f_class
            WHERE b.households_per_building IS NOT NULL
              AND cc.load_method = 'household';"""
        ...
        update_query = """
            ...
            WHERE version_id = %(v)s
              AND postcode_result_plz = %(p)s
              AND country_code = %(cc)s;"""
        self.cur.execute(update_query, {"avg": avg_val, "v": VERSION_ID, "p": plz, "cc": country_code})
    ...
    def set_settlement_type_per_plz(
        ...
        fetch_query = """
            ...
            WHERE version_id = %(v)s
              AND postcode_result_plz = %(p)s
              AND country_code = %(cc)s;"""
        self.cur.execute(fetch_query, {"v": VERSION_ID, "p": plz, "cc": country_code})
        ...
        lower_cutoff = 1 / 3
        upper_cutoff = 2 / 3
        score_deadband = max(0.0, float(settlement_type_thresholds.get("score_deadband", 0.0)))

        if abs(score - lower_cutoff) <= score_deadband or abs(score - upper_cutoff) <= score_deadband:
            settlement_type = 2
        elif score >= upper_cutoff:
            settlement_type = 3
        elif score >= lower_cutoff:
            settlement_type = 2
        else:
            settlement_type = 1
        ...
        update_query = """
            ...
            WHERE version_id = %(v)s
              AND postcode_result_plz = %(p)s
              AND country_code = %(cc)s;"""
        self.cur.execute(update_query, {"stype": settlement_type, "v": VERSION_ID, "p": plz, "cc": country_code})
        return settlement_type

    def set_building_peak_load(self) -> int:
        """
        * Sets the area, households_per_building, and peak_load in the buildings_tem table
        * Uses consumer_categories table to determine load_method (household vs area) per f_class
        * Returns count of buildings with zero load
        :return: Number of unloaded buildings in buildings_tem
        """
        # Step 1: Set area and estimate households_per_building where missing
        # Uses consumer_categories.load_method to decide: household types get household estimation,
        # area types get households_per_building = 1
        query = """
                -- Normalize unknown/empty class labels before demand mapping so they
                -- use the configured generic residential defaults (definition='yes').
                UPDATE buildings_tem b
                SET f_class = CASE
                    WHEN b.f_class IS NULL THEN 'yes'
                    WHEN NULLIF(TRIM(b.f_class), '') IS NULL THEN 'yes'
                    WHEN LOWER(TRIM(b.f_class)) IN ('unknown', 'none', 'null', 'nan', 'n_a', 'na') THEN 'yes'
                    ELSE LOWER(TRIM(b.f_class))
                END
                WHERE COALESCE(b.type, '') != 'Transformer';

                UPDATE buildings_tem b
                SET f_classes = COALESCE(NULLIF(LOWER(TRIM(b.f_classes)), ''), b.f_class)
                WHERE COALESCE(b.type, '') != 'Transformer';

                UPDATE buildings_tem b
                SET area = ST_Area(b.geom),
                    households_per_building = COALESCE(b.households_per_building,
                        CASE
                        WHEN cc.load_method = 'household' THEN
                            CASE
                            WHEN ST_Area(b.geom) < 160 THEN GREATEST(1, b.floors)
                            ELSE GREATEST(1, floor(ST_Area(b.geom) / 90) * b.floors)
                            END
                        ELSE 1
                        END
                    )
                FROM consumer_categories cc
                WHERE cc.definition = b.f_class;

                UPDATE buildings_tem b
                SET area = ST_Area(b.geom),
                    households_per_building = COALESCE(b.households_per_building, 1)
                WHERE NOT EXISTS (SELECT 1 FROM consumer_categories cc WHERE cc.definition = b.f_class);

                UPDATE buildings_tem b
                SET peak_load_in_kw = CASE
                    WHEN cc.load_method = 'household' THEN
                        b.households_per_building * COALESCE(cc.peak_load, 0)
                    WHEN cc.load_method = 'area' THEN
                        b.area * COALESCE(cc.peak_load_per_m2, 0) / 1000
                    ELSE 0
                    END
                FROM consumer_categories cc
                WHERE cc.definition = b.f_class;
                ...
                UPDATE buildings_tem b
                SET peak_load_in_kw = CASE
                    WHEN def.peak_load IS NOT NULL AND def.peak_load != 'NaN' THEN
                        b.households_per_building * def.peak_load
                    ELSE
                        b.area * COALESCE(NULLIF(def.peak_load_per_m2, 'NaN'), 29.0) / 1000
                    END
                FROM consumer_categories def
                WHERE def.definition = '_default'
                  AND NOT EXISTS (SELECT 1 FROM consumer_categories cc WHERE cc.definition = b.f_class);"""
        self.cur.execute(query)
        ...
        # Keep buildings with peak_load = 0 instead of deleting them
        # They will still be included in buildings_result for visualization

        return count

    def update_too_large_consumers_to_zero(self) -> int:
        """
        Previously set the load to zero if the peak load is too large (> 100)
        Now disabled - keep all buildings with their original peak load
        :return: number of the large customers (for logging only, no modification)
        """
        # Count large consumers for logging but don't modify them
        query = """SELECT COUNT(*)
                FROM buildings_tem b
                JOIN consumer_categories cc ON cc.definition = b.f_class
                WHERE b.peak_load_in_kw > 100
                  AND cc.load_method = 'area';"""
        self.cur.execute(query)
    ...
    def assign_close_buildings(self) -> None:
        """
        Previously set peak load to zero for buildings touching large customers
        Now disabled - keep all buildings with their original peak load
        :return:
        """
        # Disabled - no longer modifying buildings based on proximity to large customers
        return None

    def insert_transformers(self, plz: int, country_code: str = "DE") -> None:
        """
        ...
        """
        insert_query = """
                       INSERT INTO buildings_tem (osm_id, geom, plz, center, type, peak_load_in_kw)
                       SELECT t.osm_id, t.geom, %(p)s, ST_Centroid(t.geom), 'Transformer', -1
                       FROM transformers as t
                       JOIN postcode_result post ON ST_Within(t.geom, post.geom)
                       WHERE post.postcode_result_plz = %(p)s
                         AND post.version_id = %(v)s
                         AND post.country_code = %(c)s
                         AND t.country_code = %(c)s;"""
        self.cur.execute(insert_query, {"p": plz, "v": VERSION_ID, "c": country_code})

    def count_indoor_transformers(self) -> None:
        """Counts indoor transformers (the actual deletion happens in drop_indoor_transformers)"""
        # Count is now done inside drop_indoor_transformers to avoid computing ST_Union twice
        pass

    def drop_indoor_transformers(self) -> None:
        """
        Drop transformer if it is inside a building with zero load.
        Also logs the count of deleted transformers.
        :return:
        """
        ...
        self.logger.debug(f"{count} indoor transformers deleted")   
    ...
    def set_ways_tem_table_infdb(self, ways_data: list[tuple], plz: int = None) -> int:
        ...
        execute_values(
            ...
            """INSERT INTO ways_tem
               ...
               VALUES %s""",
            ways_data,
            template="(%s, %s, %s, %s, %s, ST_Transform(%s::geometry, 3035), %s)",
            page_size=500,
        )
    ...
    def set_ways_tem_table(self, plz: int, country_code: str = "DE") -> int:
        """
        * Inserts ways inside the plz area to the ways_tem table
        ...
        """
        query = """WITH post AS (
                       SELECT ST_Buffer(geom, %(buffer_m)s) AS geom
                       FROM postcode_result
                       WHERE version_id = %(v)s
                         AND postcode_result_plz = %(p)s
                         AND country_code = %(c)s
                       LIMIT 1
                   )
                   INSERT INTO ways_tem (clazz, source, target, cost, reverse_cost, geom, way_id, country_code)
                   SELECT w.clazz, w.source, w.target, w.cost, w.reverse_cost, w.geom, w.way_id, w.country_code
                   FROM ways AS w
                   JOIN post ON post.geom && w.geom
                            AND ST_Intersects(w.geom, post.geom)
                   WHERE w.country_code = %(c)s;
                   SELECT COUNT(*)
                   FROM ways_tem;"""
        self.cur.execute(
            query,
            {"v": VERSION_ID, "p": plz, "c": country_code, "buffer_m": WAYS_BOUNDARY_BUFFER_M},
        )
        count = self.cur.fetchone()[0]
    ...
    def set_vertice_id(self) -> int:
        ...
        query = """UPDATE buildings_tem b
                   SET vertice_id = sub.vid
                   FROM (
                       SELECT DISTINCT ON (b2.osm_id)
                              b2.osm_id,
                              v.id AS vid
                       FROM buildings_tem b2
                       JOIN ways_tem_vertices_pgr v
                         ON ST_DWithin(v.geom, b2.center, 1.0)
                       ORDER BY b2.osm_id, ST_Distance(v.geom, b2.center)
                   ) sub
                   WHERE b.osm_id = sub.osm_id;"""
        self.cur.execute(query)

        query2 = """UPDATE buildings_tem b
                    SET connection_point = COALESCE(
                        (SELECT w.target FROM ways_tem w WHERE w.source = b.vertice_id LIMIT 1),
                        (SELECT w.source FROM ways_tem w WHERE w.target = b.vertice_id LIMIT 1),
                        b.vertice_id
                    )
        ...
        # Keep buildings without connection points instead of deleting them
        # They will still be included in buildings_result for visualization
        # delete_query = """DELETE
        #                   FROM buildings_tem
        #                   WHERE connection_point IS NULL
    ...
    def insert_equipment_data(self, equipment_df: pd.DataFrame):
        ...
        int_cols = ['s_max_kva', 'max_i_a', 'r_mohm_per_km', 'x_mohm_per_km',
                    'z_mohm_per_km', 'cost_eur', 'equipment_only_cost_eur', 'installed_cost_eur']
        for col in int_cols:
            if col in equipment_df.columns:
                equipment_df[col] = pd.to_numeric(equipment_df[col], errors='coerce').astype('Int64')

        if 'equipment_only_cost_eur' not in equipment_df.columns:
            equipment_df['equipment_only_cost_eur'] = equipment_df.get('cost_eur')
        else:
            equipment_df['equipment_only_cost_eur'] = equipment_df['equipment_only_cost_eur'].where(
                equipment_df['equipment_only_cost_eur'].notna(),
                equipment_df.get('cost_eur'),
            )
        if 'installed_cost_eur' not in equipment_df.columns:
            equipment_df['installed_cost_eur'] = None
        ...
        for _, r in equipment_df.iterrows():
            rows.append((
                ...
                r.get('z_mohm_per_km'),
                r.get('cost_eur'),
                r.get('equipment_only_cost_eur'),
                r.get('installed_cost_eur'),
                r.get('typ')
            ))
        execute_values(
            self.cur,
